...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import fzf_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function dev_jll()
        ...
        override_dir = joinpath(Pkg.devdir(), "fzf_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"fzf", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "fzf_jll", "src", "fzf_jll.jl"))
        @info("fzf_ll dev'ed out to /depot/dev/fzf_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("214eeab7-80f7-51ab-84ad-2988db7cef09"))
platforms = [Pkg.Artifacts.unpack_platform(e, "fzf", artifacts_toml) for e in artifacts["fzf"]]
...
if best_platform === nothing
    @debug("Unable to load fzf; unsupported platform $(triplet(platform_key_abi()))")
else
